...
void main() {
    ...
    vec3 viewNormal = normalize(interpolatedNormal);                              // HINT: Needs to be in VCS
    fresnel = abs(dot(viewDirection, viewNormal)); //calculating the fresnel, need abs since we cannot have negative sillouettes
